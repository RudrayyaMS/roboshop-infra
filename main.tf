...
#
#module "elasticache" {
#  source = "git::https://github.com/RudrayyaMS/tf-module-elasticache.git"
#  env = var.env
#  tags = var.tags
#
#  subnet_ids = local.db_subnet_ids
...
module "alb" {
  source = "git::https://github.com/RudrayyaMS/tf-module-alb.git"
  env = var.env
  tags = var.tags
...
module "app" {
  source = "git::https://github.com/RudrayyaMS/tf-module-app.git"
  env    = var.env
  tags   = var.tags
  ...
  alb_dns_name     = lookup(lookup(lookup(module.alb, each.value["alb"], null ), "listener" , null), "arn" ,null)
}
output "alb" {
  value = module.alb
}
